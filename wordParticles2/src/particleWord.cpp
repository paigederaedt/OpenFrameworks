...
void particleWord::draw(){
    ...
    float newSize = ofMap(size, 0, ofGetElapsedTimef(), 12, 200);
    ttfword.loadFont("Lekton-Regular.ttf", (int) newSize);
...
particleWord::particleWord(){
    ...
    color.a = ofMap(sin(ofGetElapsedTimef()), -1, 1, 50, 400);
...
void particleWord::bounceOffWalls(){
    ...
    float maxy = ofGetHeight(); //+ ttfword.stringHeight(word);
